...
const ImportHeadquarterModal = ({ onClose, onImported }) => {
  ...
  const downloadTemplate = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "Headquarters");
    XLSX.writeFile(workbook, "headquarters_template.xlsx");
  };
  ...
  const handleImport = async () => {
    ...
    setResult(null);
    try {
      const data = await importHeadquartersFromExcel(file, fromRow, toRow);
      setResult(data);
      toast.success(data.message);
      onImported?.();
    } catch (error) {
      console.error(error);
      toast.error("Failed to import headquarters");
    } finally {
      setImporting(false);
    }
  };
          ...
          <div className="mb-5 p-3 bg-gray-50 border border-gray-200 rounded-lg text-sm space-y-2 max-h-64 overflow-y-auto">
            <p className="text-gray-700">
              <span className="font-semibold">
                {result.summary?.insertedHQCount ?? 0}
              </span>{" "}
              headquarter(s),{" "}
              <span className="font-semibold">
                {result.summary?.insertedAreaCount ?? 0}
              </span>{" "}
              area(s),{" "}
              <span className="font-semibold">
                {result.summary?.insertedDoctorCount ?? 0}
              </span>{" "}
              doctor(s) imported
            </p>
                ...
                <ul className="space-y-1">
                  {result.duplicates.doctors.map((dup, idx) => (
                    ...
                    >
                      ...
                      {dup.headquarter} / {dup.area}) — {dup.reason}
                    </li>
                  ))}
                </ul>
              </div>
...
export default ImportHeadquarterModal;
